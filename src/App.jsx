...
function FollowsTab({account,follows,onlinePlayers,onToggle,onLogin}){
 ...
 return <div className="follows-list">
  {aiFollows.length>0&&<><h4>AI 侠客</h4>{aiFollows.map(f=>{const a=AGENTS.find(x=>x.id===f.targetId);if(!a)return null;return <article key={f.targetId}><Avatar agent={a} size={40}/><div><small>AI 侠客 · {a.role}</small><p>{a.name}</p></div><button className="secondary-button" onClick={()=>onToggle('ai',a.id)}>取消关注</button></article>;})}</>}
  {playerFollows.length>0&&<><h4>联机侠客</h4>{playerFollows.map(f=>{const online=onlinePlayers.find(p=>p.id===f.targetId);return <article key={f.targetId}><Avatar agent={{color:online?.color||'#427ab5'}} size={40}/><div><small>{online?'在线 · '+online.name:'当前离线'}</small><p>{online?.name||'联机侠客'}</p></div><button className="secondary-button" onClick={()=>onToggle('player',f.targetId)}>取消关注</button></article>;})}</>}
 </div>;
}
